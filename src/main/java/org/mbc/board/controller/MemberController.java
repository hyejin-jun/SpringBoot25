...
public class MemberController {
    ...
    @GetMapping("/update")  // http://localhost/member/join
    public void updateGet() {
        // void -> templates/member/join.html
        log.info("넘어간드아아아아아ㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏㅏ");
        // url이 넘어오면 프론트로 페이지 출력용
    }

    @PostMapping("/update") // http://localhost/member/join post메서드로 처리됨
    public String updatePost(MemberJoinDTO memberJoinDTO, RedirectAttributes redirectAttributes) {
        // html에서 넘어오는 데이터 처리용
        ...
        try {
            ...
            redirectAttributes.addFlashAttribute("error", "mid");
            // 회원가입시 id 중복되는 예외처리
            return "redirect:/member/login"; // 회원가입 페이지로 다시 감.

        }
        redirectAttributes.addFlashAttribute("result", "success");
        return "redirect:/board/list"; //회원가입 성공시 보드 화면으로 이동
